...
```dax
Total Revenue = SUM(cars_dataset_cleaned[sellingprice])

Units Sold = COUNTROWS(cars_dataset_cleaned)

Avg Selling Price = AVERAGE(cars_dataset_cleaned[sellingprice])

Avg Condition = AVERAGE(cars_dataset_cleaned[condition])

Best Month Name =
VAR MonthSummary =
    SUMMARIZE(
        cars_dataset_cleaned,
        cars_dataset_cleaned[saledate].[MonthNo],
        "Rev", [Total Revenue]
    )
VAR BestMonthNo = MAXX(TOPN(1, MonthSummary, [Rev], DESC),
                    cars_dataset_cleaned[saledate].[MonthNo])
RETURN
    FORMAT(DATE(2024, BestMonthNo, 1), "MMMM")

Best Quarter =
VAR QtrSummary =
    SUMMARIZE(
        cars_dataset_cleaned,
        cars_dataset_cleaned[saledate].[QuarterNo],
        "Rev", [Total Revenue]
    )
VAR BestQtr = MAXX(TOPN(1, QtrSummary, [Rev], DESC),
                cars_dataset_cleaned[saledate].[QuarterNo])
RETURN
    "Q" & BestQtr
...
MAXX(
    TOPN(1,
        SUMMARIZE(cars_dataset_cleaned, cars_dataset_cleaned[make], "Rev", [Total Revenue]),
        [Rev], DESC),
    cars_dataset_cleaned[make]
)

Top State =
MAXX(
    TOPN(1,
        SUMMARIZE(cars_dataset_cleaned, cars_dataset_cleaned[state], "Units", [Units Sold]),
        [Units], DESC),
    UPPER(cars_dataset_cleaned[state])
)
```
...
Price Tier =
SWITCH(TRUE(),
    cars_dataset_cleaned[sellingprice] < 15000, "Budget",
    cars_dataset_cleaned[sellingprice] < 30000, "Mid-Range",
    "Premium/Luxury"
)

Price Tier Sort =
SWITCH(TRUE(),
    cars_dataset_cleaned[Price Tier] = "Budget", 1,
    cars_dataset_cleaned[Price Tier] = "Mid-Range", 2,
    3
)

Quarter Label = "Q" & FORMAT(cars_dataset_cleaned[saledate], "Q")

Condition Rounded = ROUND(cars_dataset_cleaned[condition], 0)

State Upper = UPPER(cars_dataset_cleaned[state])
```
